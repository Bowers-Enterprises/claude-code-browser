...
import {
  ViewVisibilityManager,
  PANEL_LABELS,
} from "./services/viewVisibilityManager";
...
export async function activate(
  ...
  try {
    ...
    // Create skill update service for provenance tracking
    const skillUpdateService = new SkillUpdateService(context);

    // Create providers with folder manager
    const skillsProvider = new SkillsProvider(folderManager);
    skillsProvider.setUpdateService(skillUpdateService);
    ...
    const hookManagerTreeView = vscode.window.createTreeView(
      "claudeCodeBrowser.hookManager",
      {
        treeDataProvider: hookManagerProvider,
      },
    );
    const agentTeamsTreeView = vscode.window.createTreeView(
      "claudeCodeBrowser.agentTeams",
      {
        treeDataProvider: agentTeamProvider,
      },
    );
    const worktreeTreeView = vscode.window.createTreeView(
      "claudeCodeBrowser.worktrees",
      {
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand(
        "claudeCodeBrowser.refreshHookManager",
        () => hookManagerProvider.refresh(),
      ),
      vscode.commands.registerCommand(
        "claudeCodeBrowser.refreshAgentTeams",
        () => agentTeamProvider.refresh(),
      ),
      vscode.commands.registerCommand(
        "claudeCodeBrowser.refreshWorktrees",
        () => worktreeProvider.refresh(),
      ),
      vscode.commands.registerCommand(
        "claudeCodeBrowser.agentTeams.watch",
        (item: TeamTreeItem) => {
    ...
    skillWatcher.start((skillName) => {
      vscode.window
        .showInformationMessage(`New skill "${skillName}" created!`, "Preview")
        .then((choice) => {
          if (choice === "Preview") {
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand(
        "claudeCodeBrowser.managePanels",
        async () => {
      ...
      vscode.commands.registerCommand(
        "claudeCodeBrowser.showAllPanels",
        async () => {
    ...
    vscode.window.showErrorMessage(
      `Claude Code Browser failed to activate: ${error}`,
    );
