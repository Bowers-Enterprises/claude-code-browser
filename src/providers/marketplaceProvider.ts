...
const STAFF_PICKS: MarketplaceSkill[] = [
  // Productivity & Automation
  {
    ...
    description:
      "Complete GitHub workflow automation: create PRs, manage issues, run API queries",
    author: "daymade",
  ...
  },
  {
    ...
    description:
      "Write and execute browser automation tests with visual testing across viewports",
    author: "lackeyjb",
  ...
  },
  {
    ...
    name: "Markdown Tools",
    description:
      "Convert documents (Word, PDF, PowerPoint) to markdown with image extraction",
    author: "daymade",
    githubUrl: "https://github.com/daymade/claude-code-skills",
  ...
  },
  {
    ...
    description:
      "Extract Mermaid diagrams from markdown and convert to PNG images",
    author: "daymade",
    githubUrl: "https://github.com/daymade/claude-code-skills",
    category: "documentation",
    stars: 1200,
  ...
  },
  {
    ...
    description:
      "Create presentation decks with data-driven charts and PPTX generation",
    author: "daymade",
    githubUrl: "https://github.com/daymade/claude-code-skills",
    category: "documentation",
    stars: 1200,
  ...
  },
  {
    ...
    description:
      "Comprehensive QA testing infrastructure following Google Testing Standards",
    author: "daymade",
    githubUrl: "https://github.com/daymade/claude-code-skills",
    category: "testing",
    stars: 1200,
  ...
  },
  {
    ...
    name: "Skill Creator",
    description:
      "Meta-skill for building and packaging custom Claude Code skills",
    author: "daymade",
    githubUrl: "https://github.com/daymade/claude-code-skills",
  ...
  },
  {
    ...
    description:
      "Generate animated CLI demos and terminal recordings for documentation",
    author: "daymade",
    githubUrl: "https://github.com/daymade/claude-code-skills",
    category: "documentation",
    stars: 1200,
 ...
 */
export type MarketplaceTreeItem =
  | CategoryItem
...
export class MarketplaceProvider implements vscode.TreeDataProvider<MarketplaceTreeItem> {
  private _onDidChangeTreeData = new vscode.EventEmitter<
    MarketplaceTreeItem | undefined | null | void
  >();
   ...
   */
  async getChildren(
    element?: MarketplaceTreeItem,
  ): Promise<MarketplaceTreeItem[]> {
    ...
    if (!element) {
      ...
      for (const source of sources) {
        try {
          ...
          console.error(
            `Error loading skills for source ${source.name}:`,
            error,
          );
    ...
    if (element instanceof SourceItem) {
      try {
        ...
        console.error(
          `Error loading skills for source ${element.source.name}:`,
          error,
        );
        vscode.window.showErrorMessage(
          `Failed to load skills from ${element.source.name}: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
        return [];
      }
    }
   ...
   */
  private async getSkillsForSource(
  ...
  private getStaffPicksCategories(): CategoryItem[] {
    ...
    for (const [category, count] of categoryCounts.entries()) {
      if (count > 0) {
        // Only show categories with skills
   ...
   */
  private applyFilterToMarketplaceSkills(
    ...
    return skills.filter((skill) =>
      this.matchesFilterForMarketplaceSkill(skill),
    );
  }
   ...
   */
  private applyFilterToSourceSkills(
  ...
  private matchesFilterForSourceSkill(skill: SkillDefinition): boolean {
    ...
    return (
      ...
      skill.description.toLowerCase().includes(this.filterText) ||
      Boolean(
        skill.author && skill.author.toLowerCase().includes(this.filterText),
      )
    );
  }
}
...
async function installSkill(
  skill: MarketplaceSkill | SkillDefinition,
  ...
  await vscode.window.withProgress(
    ...
    async (progress) => {
      try {
        ...
        await execFileAsync(
          "git",
          ["clone", "--depth", "1", skill.githubUrl, tempDir],
          {
        ...
        if (marketplaceSkill.skillPath) {
          ...
          try {
            ...
            throw new Error(
              `Skill directory "${marketplaceSkill.skillPath}" not found in repository`,
            );
          }
        } else {
        ...
        // Stamp provenance for update tracking
        if (updateService) {
          const marketplaceSkill = skill as MarketplaceSkill;
          const sourceSkill = skill as SkillDefinition;
          await updateService.stampInstall(targetPath, {
            source: sourceSkill.sourceId ? "marketplace" : "github",
            repo: skill.githubUrl,
            skillPath: marketplaceSkill.skillPath,
            marketplaceSourceId: sourceSkill.sourceId,
          });
        }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      async (item: MarketplaceSkillItem) => {
        await installSkill(item.skill, updateService);
        provider.refresh();
      },
    ),
  );
        ...
        if (url) {
          try {
            ...
            vscode.window.showInformationMessage(
              `Added marketplace source: ${source.name}`,
            );
            provider.refresh();
          } catch (error) {
            vscode.window.showErrorMessage(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      async (item: SourceItem) => {
        ...
        if (choice === "Remove") {
          const removed = await provider["sourceManager"].removeSource(
            item.source.id,
          );
          if (removed) {
            vscode.window.showInformationMessage(
              `Removed marketplace source: ${item.source.name}`,
            );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      async (item: SourceItem) => {
        ...
        provider.refresh();
        vscode.window.showInformationMessage(
          `Refreshed source: ${item.source.name}`,
        );
      },
    ),
  );
